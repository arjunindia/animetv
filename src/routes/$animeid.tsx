...
const EpisodeComponent = ({ episode }: { episode: IAnimeEpisode }) => {
  ...
  return (
    ...
    >
      <div
        className={`group-focus:scale-105 ${focused ? "scale-105 outline outline-3" : ""} w-[30vw] p-3 ${episode.isFiller ? `bg-red-600` : `bg-popover-foreground`} text-popover mb-2 rounded relative`}
      >
...
const BackButton = () => {
  ...
  return (
    <Link
      ref={ref}
      to=".."
      className={`bg-background shadow-xl rounded-full p-3 h-min m-3 mt-5 ${focused ? "outline outline-ring scale-110 outline-4" : ""}`}
    >
      <ArrowLeft />
    </Link>
  );
